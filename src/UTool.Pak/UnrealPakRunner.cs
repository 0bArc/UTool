...
public static class UnrealPakRunner
{
    public static string ResolveExecutable(UnrealPakOptions? options = null)
    {
        ...
        var fromEnv = Environment.GetEnvironmentVariable("UTOOL_UNREALPAK");
        ...
        var root = UnrealPakToolchain.DefaultInstallRoot;
        var common = new[]
        {
            Path.Combine(root, "Engine", "Binaries", "Win64", "UnrealPak.exe"),
            Path.Combine(root, "Binaries", "Win64", "UnrealPak.exe"),
        ...
        throw new FileNotFoundException(
            "UnrealPak.exe not found. Ensure assets/UnrealPak.zip exists in the utool repo " +
            "(auto-extracts on first use), run 'utool setup unrealpak', or set UTOOL_UNREALPAK.");
    }
        ...
        UnrealPakOptions? options = null)
    {
        ...
        var response = Path.Combine(
            Path.GetTempPath(),
            "utool-" + Guid.NewGuid().ToString("N") + ".txt");
        try
        {
