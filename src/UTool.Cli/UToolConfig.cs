...
public sealed class UToolConfig
{
    ...
    // Legacy utool.json keys (read-only aliases)
    ...
    public string ResolveDataPak(string? gameId = null)
    {
        ...
        throw new InvalidOperationException(
            "dataPak not configured. Set dataPak or gamePaksDir in utool.json, or mod.json pak.sourcePak to a file path.");
    }
    ...
    public string ResolvePlayerDataDir(string? gameId = null)
    {
        ...
        {
            throw new InvalidOperationException(
                "playerDataDir not configured and no gameId. Set playerDataDir in utool.json, games.<id>.playerDataDir, mod.json target.gameId, or UTOOL_PLAYER_DATA.");
        }
    ...
    public static UToolConfig Load(string? startDirectory = null)
    {
        var dir = startDirectory ?? Directory.GetCurrentDirectory();
        for (var i = 0; i < 8 && !string.IsNullOrEmpty(dir); i++)
        {
            foreach (var fileName in new[] { "utool.json", "csstratware.json" })
            {
                ...
                var cfg = JsonSerializer.Deserialize<UToolConfig>(json, JsonOptions)
                    ?? new UToolConfig();
