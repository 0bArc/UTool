...
public sealed class ModBuildCleanupTests
{
    ...
    {
        var path = Path.Combine(Path.GetTempPath(), "utool-missing-" + Guid.NewGuid().ToString("N"));
    ...
    {
        var modDir = Path.Combine(Path.GetTempPath(), "utool-mod-" + Guid.NewGuid().ToString("N"));
